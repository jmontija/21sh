...
/*   Updated: 2016/06/17 03:30:05 by julio            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	manage_env(t_group *grp)
{
	...
	while (++i < grp->define_cmd[namenv] && CMD(cmd_split)[i + grp->define_cmd[e_opt] + 1])
		grp->env[pos + i] = SDUP(CMD(cmd_split)[i + grp->define_cmd[e_opt] + 1]);
}
...
void	define_arg_cmd(t_group *grp)
{
	...
	while (CMD(cmd_split) && CMD(cmd_split)[++i])
	{
		...
		else if (is_env(CMD(cmd_split)[i]) > 0 && grp->define_cmd[utils] == false)
...
int		manage_pre_exec(int idx_cmd, t_group *grp, char *pipe_cmd)
{
	...
	define_arg_cmd(grp);
	manage_env(grp);
	redir_from(idx_cmd, grp); // top | cat
